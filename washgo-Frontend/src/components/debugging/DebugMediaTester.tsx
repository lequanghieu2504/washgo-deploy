...
// Simple DTO matching backend response
...
export default function DebugMediaTester() {
  ...
  const absUrl = (u?: string) => (!u ? "" : u.startsWith("/") ? `${apiBase.replace(/\/+$/, "")}${u}` : u);
  ...
  // CARWASH: UPLOAD
  // -----------------------
  const [carwashId, setCarwashId] = useState<number>(1);
  const carwashFileRef = useRef<HTMLInputElement | null>(null);
  const [carwashCover, setCarwashCover] = useState<boolean>(false);
  const [carwashSort, setCarwashSort] = useState<number>(0);
  const [visibility, setVisibility] = useState<"PUBLIC" | "PRIVATE">("PUBLIC");
  const [carwashProgress, setCarwashProgress] = useState<number>(0);
  const [carwashLog, setCarwashLog] = useState<string>("");

  // -----------------------
  // CARWASH: LIST
  // -----------------------
  const [listCarwashId, setListCarwashId] = useState<number>(1);
  ...
  const [avatarDTO, setAvatarDTO] = useState<MediaDTO | null>(null); // ✅ giữ URL avatar vừa up
  ...
  const [feedbackCover, setFeedbackCover] = useState<boolean>(false);
  const [feedbackSort, setFeedbackSort] = useState<number>(0);
  const [feedbackVisibility, setFeedbackVisibility] = useState<"PUBLIC" | "PRIVATE">("PRIVATE");
  ...
  // FEEDBACK: LIST
  // -----------------------
  const [listFeedbackId, setListFeedbackId] = useState<number>(1);
  const [feedbackItems, setFeedbackItems] = useState<MediaDTO[]>([]);
  const [listFeedbackLog, setListFeedbackLog] = useState<string>("");

  // -----------------------
  // FEEDBACK: DELETE feedback entity
  // -----------------------
  const [delFeedbackId, setDelFeedbackId] = useState<number>(1);
  const [delFeedbackLog, setDelFeedbackLog] = useState<string>("");

  // note
  const baseNote = useMemo(() => `API base: ${apiBase || "(same origin)"}`, [apiBase]);
  ...
  const handleUploadCarwash = async () => {
    ...
    const url = api(
      `/api/media/carwash/${encodeURIComponent(String(carwashId))}?cover=${carwashCover}&sortOrder=${carwashSort}&visibility=${visibility}`
    );
    const fd = new FormData();
    fd.append("file", f);

    try {
      const resp = await xhrUpload(url, fd, setCarwashProgress, buildAuthHeaders());
      if (!resp.ok) {
        const t = await resp.text();
        throw new Error(`HTTP ${resp.status}: ${t}`);
      }
      const data = (await resp.json()) as MediaDTO;
  ...
  const handleListCarwash = async () => {
    ...
    try {
      const resp = await fetch(api(`/api/media/carwash/${encodeURIComponent(String(listCarwashId))}`), {
  ...
  const onListCarwashEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      void handleListCarwash();
    }
  };
  ...
  const handleDeleteCarwashMedia = async (mediaId: string) => {
    if (!listCarwashId) return alert("Thiếu carwashId để xoá");
    if (!confirm(`Xoá media ${mediaId} khỏi Carwash ${listCarwashId}?`)) return;
    try {
      const resp = await fetch(api(`/api/media/carwash/${encodeURIComponent(String(listCarwashId))}/${mediaId}`), {
        method: "DELETE",
        headers: buildAuthHeaders(),
      });
  ...
  const handleUploadAvatar = async () => {
    ...
    const url = api(`/api/media/users/${encodeURIComponent(String(userId))}/avatar`);
    const fd = new FormData();
    fd.append("file", f);

    try {
      const resp = await xhrUpload(url, fd, setAvatarProgress, buildAuthHeaders());
      if (!resp.ok) {
        const t = await resp.text();
        throw new Error(`HTTP ${resp.status}: ${t}`);
      }
  ...
  const handleDeleteAvatar = async () => {
    ...
    try {
      const resp = await fetch(api(`/api/media/users/${encodeURIComponent(String(userId))}/avatar`), {
        method: "DELETE",
        headers: buildAuthHeaders(),
      });
      if (!resp.ok && resp.status !== 204) {
  ...
  const handleUploadFeedback = async () => {
    ...
    const url = api(
      `/api/feedback/${encodeURIComponent(String(feedbackId))}/media?cover=${feedbackCover}&sortOrder=${feedbackSort}&visibility=${feedbackVisibility}`
    );
    const fd = new FormData();
    fd.append("file", f);

    try {
      const resp = await xhrUpload(url, fd, setFeedbackProgress, buildAuthHeaders());
      if (!resp.ok) {
        const t = await resp.text();
        throw new Error(`HTTP ${resp.status}: ${t}`);
      }
  ...
  const handleListFeedback = async () => {
    ...
    try {
      const resp = await fetch(api(`/api/feedback/${encodeURIComponent(String(listFeedbackId))}/media`), {
        headers: buildAuthHeaders(),
      });
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const data = (await resp.json()) as MediaDTO[];
  ...
  const onListFeedbackEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      void handleListFeedback();
    }
  };
  ...
  const handleDeleteFeedbackMedia = async (mediaId: string) => {
    if (!listFeedbackId) return alert("Thiếu feedbackId để xoá");
    if (!confirm(`Xoá media ${mediaId} khỏi Feedback ${listFeedbackId}?`)) return;
    try {
      const resp = await fetch(api(`/api/media/feedback/${encodeURIComponent(String(listFeedbackId))}/${mediaId}`), {
        method: "DELETE",
        headers: buildAuthHeaders(),
      });
      if (!resp.ok && resp.status !== 204) {
  ...
  const handleDeleteFeedbackEntity = async () => {
    ...
    setDelFeedbackLog("");
    try {
      const resp = await fetch(api(`/api/feedback/${encodeURIComponent(String(delFeedbackId))}`), {
        method: "DELETE",
        headers: buildAuthHeaders(),
      });
      if (!resp.ok && resp.status !== 204) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Test: <code className="bg-gray-100 px-1 rounded">/api/media/carwash/:id</code>,
        <code className="bg-gray-100 px-1 rounded ml-1">/api/media/users/:id/avatar</code>,
        <code className="bg-gray-100 px-1 rounded ml-1">/api/feedback/:id/media</code>,
        <code className="bg-gray-100 px-1 rounded ml-1">/api/media/serve/:id</code>
      </p>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label className="inline-flex items-center gap-2 text-sm text-gray-700">
              <input type="checkbox" checked={carwashCover} onChange={(e) => setCarwashCover(e.target.checked)} />
              Cover
            </label>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input ref={carwashFileRef} type="file" accept="image/*,video/*" className="block" />
            {carwashProgress > 0 && (
              <div className="flex items-center gap-2">
                <progress value={carwashProgress} max={100} className="w-56" />
                <span className="text-sm text-gray-600">{carwashProgress}%</span>
              </div>
            )}
            <button onClick={handleUploadCarwash} className="border px-4 py-2 rounded bg-white hover:bg-gray-50">
              Upload
            </button>
          </div>
          <pre className="bg-gray-50 border rounded p-3 text-sm whitespace-pre-wrap min-h-[20px]">{carwashLog}</pre>
        </div>
      </section>

      {/* LIST CARWASH MEDIA */}
      <section className="border rounded-2xl p-4 mb-6 shadow-sm">
        <h2 className="text-lg font-medium mb-4">Xem & xoá ảnh/video Carwash</h2>
        <div className="flex items-center gap-3 flex-wrap mb-3">
          ...
          />
          <button onClick={handleListCarwash} className="border px-4 py-2 rounded bg-white hover:bg-gray-50">
            Tải danh sách
          </button>
        </div>
        ...
        <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          {items.map((m) => (
            <div key={m.id} className="border rounded-xl p-3">
              {m.mediaType?.toUpperCase() === "VIDEO" || (m.mime || "").startsWith("video/") ? (
                <video src={absUrl(m.url)} controls className="w-full h-44 object-cover rounded-md bg-gray-100" />
              ) : (
                <img src={absUrl(m.url)} alt={m.id} className="w-full h-44 object-cover rounded-md bg-gray-100" />
              )}
              <div className="text-xs text-gray-600 mt-2">
                {m.mediaType} • {m.mime || ""} • {fmtBytes(m.sizeBytes)} {m.cover ? "• COVER" : ""}
              </div>
              <div className="flex gap-2 mt-2">
                <a href={absUrl(m.url)} target="_blank" rel="noreferrer" className="text-blue-600 text-xs underline">
                  Mở
                </a>
                <button
        ...
        {items.length === 0 && <div className="text-sm text-gray-500">Chưa có media nào cho Carwash này.</div>}
      </section>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              onChange={(e) => setFeedbackVisibility(e.target.value as any)}
            >
              <option value="PUBLIC">PUBLIC</option>
              <option value="PRIVATE">PRIVATE</option>
            </select>
            <label className="inline-flex items-center gap-2 text-sm text-gray-700">
              <input type="checkbox" checked={feedbackCover} onChange={(e) => setFeedbackCover(e.target.checked)} />
              Cover
            </label>
            <label className="text-sm text-gray-700">Sort</label>
            <input
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input ref={feedbackFileRef} type="file" accept="image/*,video/*" className="block" />
            {feedbackProgress > 0 && (
              <div className="flex items-center gap-2">
                <progress value={feedbackProgress} max={100} className="w-56" />
                <span className="text-sm text-gray-600">{feedbackProgress}%</span>
              </div>
            )}
            <button onClick={handleUploadFeedback} className="border px-4 py-2 rounded bg-white hover:bg-gray-50">
              Upload
            </button>
          </div>
          <pre className="bg-gray-50 border rounded p-3 text-sm whitespace-pre-wrap min-h-[20px]">{feedbackLog}</pre>
        </div>
      </section>

      {/* FEEDBACK: LIST + DELETE MEDIA */}
      <section className="border rounded-2xl p-4 mb-6 shadow-sm">
        <h2 className="text-lg font-medium mb-4">Xem & xoá ảnh/video Feedback</h2>
        <div className="flex items-center gap-3 flex-wrap mb-3">
          <label className="text-sm text-gray-700">Feedback ID</label>
          <input
            type="number"
          ...
          />
          <button onClick={handleListFeedback} className="border px-4 py-2 rounded bg-white hover:bg-gray-50">
            Tải danh sách
          </button>
        </div>
        ...
        {feedbackItems.length === 0 && <div className="text-sm text-gray-500">Chưa có media nào cho Feedback này.</div>}
      </section>
      ...
      <section className="border rounded-2xl p-4 mb-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          />
          <button onClick={handleDeleteFeedbackEntity} className="border px-4 py-2 rounded bg-white hover:bg-gray-50 text-red-600">
            Xoá feedback
          </button>
      ...
      <section className="border rounded-2xl p-4 mb-6 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input ref={avatarFileRef} type="file" accept="image/*" className="block" />
          {avatarProgress > 0 && (
            <div className="flex items-center gap-2">
          ...
          <button onClick={handleUploadAvatar} className="border px-4 py-2 rounded bg-white hover:bg-gray-50">
            Upload avatar
          </button>
          <button onClick={handleDeleteAvatar} className="border px-4 py-2 rounded bg-white hover:bg-gray-50 text-red-600">
            Xoá avatar
          </button>

          {/* ✅ Preview avatar vừa upload */}
          {avatarDTO?.url && (
            <img src={absUrl(avatarDTO.url)} alt="avatar" className="w-20 h-20 object-cover rounded-full border" />
          )}
        </div>
        <pre className="bg-gray-50 border rounded p-3 text-sm whitespace-pre-wrap min-h-[20px] mt-3">{avatarLog}</pre>
      </section>

      <p className="text-xs text-gray-500">
        Mẹo: endpoint có @PreAuthorize thì nhớ paste JWT vào ô Authorization. Nếu backend dùng session/CSRF thì cần cấu
        hình thêm.
      </p>
    </div>
  );
