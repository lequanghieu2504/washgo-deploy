...
REM === Define locations for 5 carwashes ===
set locations[1]=456 Le Loi, Quan 1, Ho Chi Minh City
set locations[2]=929 Tran Hung Dao, Quan 5
set locations[3]=375 Hai Ba Trung, Quan 3
set locations[4]=525 Nguyen Dinh Chieu, Quan 3
set locations[5]= 797 Cach Mang Thang Tam, Phuong Nhieu Loc

REM === 1. Register admin, client, and 5 carwashes ===
...
REM === 3. Create 5 product masters ===
for /L %%i in (1,1,5) do (
...
for /L %%c in (1,1,5) do (
    ...
    REM Create 3 products for this carwash (assume product master IDs 1-5)
    for /L %%p in (1,1,3) do (
        ...
        REM BUG FIX: Changed loop variable from %%p to %%i to avoid overwriting the outer loop counter
        for /f "delims=" %%i in ('powershell -Command "(Get-Content product.json | ConvertFrom-Json).id"') do set "PRODUCT_ID=%%i"

        REM Create 3 sub-products for this product
        for /L %%s in (1,1,3) do (
